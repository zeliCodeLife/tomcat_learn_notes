...
分开的好处在于，tomcat一个目录，你的应用单独一个目录，完全分开了，想升级tomcat的时候，只需要替换一个tomcat文件夹就全部升级了。各自的conf，比如jvm参数可以在各自的文件夹中配置。
...
value是输出日志，如下：注意测试的时候，要改本机的host文件，才能转换为ip。下一个hostswitch就可以解决。
